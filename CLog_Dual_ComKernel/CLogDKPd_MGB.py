...
def apply_CLogDKPd_MGB(database, eta, d, error_graph=True, accuracy=True, plot=True, display_confusion_matrix=True):
    ...
    while t < 2000 and error(p, y) > 0.025:
        ...
        p = []
        for i in range(N):
            p.append(sigmoid(sum([alpha[l]*dp_matrix[l][i] for l in range(N)])))
    ...
    if display_confusion_matrix:
        cm_p = [1 if prob > 0.5 else 0 for prob in p]
